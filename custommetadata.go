...
func customMetadataToC(customMetadata uplink.CustomMetadata) C.CustomMetadata {
	...
	entries := (*C.CustomMetadataEntry)(C.calloc(C.sizeof_CustomMetadata, C.size_t(len(sorted))))
	custom := C.CustomMetadata{
		entries: entries,
		count:   C.uint64_t(len(sorted)),
	}
	fmt.Println("ALLO ENTRIES:", custom)
	...
	*(*reflect.SliceHeader)(unsafe.Pointer(&array)) = reflect.SliceHeader{
	...
	for i, kv := range sorted {
		...
		fmt.Println("ENTRY:", array[i], fmt.Sprintf("%p", ckey))
	}
...
func free_custom_metadata_data(custom *C.CustomMetadata) {
	...
	defer C.free(unsafe.Pointer(custom.entries))

	fmt.Println("FREE ENTRIES:", custom)
